...
<div class="page-concent-div">
  <form id="activity_form" class="layui-form" action="" method="post">
  	<input type="hidden" value="admin_edit_admin" name="post_type">
  	<input type="hidden" value="${requestScope.adminInfo.adminId }" name="admin_id">
      <div class="layui-form-item">
	    <label class="layui-form-label">管理员名称:</label>
	    <div class="layui-input-inline">
	      <input type="text" name="admin_name" value="${requestScope.adminInfo.adminName}" lay-verify="required" placeholder="请输入管理员名称" autocomplete="off" class="layui-input">
	    </div>
	  </div> 
	  <div class="layui-form-item">
	    ...
	    <div class="layui-input-block wd-100px">
	      <select name="admin_role_id" lay-filter="aihao" lay-verify="required">
    		<c:forEach items="${requestScope.adminRoleList}" var="adminRole">
    			<c:choose>
    				<c:when test="${requestScope.adminInfo.adminRoleId == adminRole.adminRoleId}">
    			        <option selected="" value="${adminRole.adminRoleId}" >${adminRole.adminRoleName}</option>
    				</c:when>
    				<c:otherwise>
...
layui.use(['form', 'layedit', 'laydate'], function(){
  ...
  form.on('submit(demo1)', function(data){
  	console.log(data.field);
   	is_ok("${pageContext.request.contextPath}/route", data.field, 'editAdminReturn');
    return false;
  });
});
// 修改管理员回调函数
editAdminReturn = function (returnData) {
